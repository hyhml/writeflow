...
通过条件：
- 四项全部 >= 5 才能通过。
- 任一项 < 5 即失败，pass_reason 必须是 "shallow_dimensions"。
- 即使四项 >= 5，只要任何关键 depth_questions 的 status 是 "missing"，也不能通过。
- status 是 "not_deep_enough" 时应给出改进建议，但不再单独阻断通过。
- 全部 >= 5 且没有 missing 的关键问题时，pass_reason 必须是 "depth_passed"。
...
输出 JSON。不要因为语言顺、术语多、结构完整就给高分；只看它是否真正不浅，是否把真实新意资产讲透。"""
    ...
    def _normalize_evaluation(self, result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        failed = [dimension for dimension, score in scores.items() if score < MIN_PASSING_SCORE]
